...
class CeremonialInstrumentalClassification(Node):
    ...
    problem_solving_contribution: Optional[float] = (
        None  # Contribution to problem solving (0-1)
    )
    status_quo_reinforcement: Optional[float] = (
        None  # Reinforces existing patterns (0-1)
    )
...
class ValueSystem(Node):
    ...
    ceremonial_elements: List[str] = field(
        default_factory=lambda: []
    )  # Status quo reinforcing aspects
    instrumental_elements: List[str] = field(
        default_factory=lambda: []
    )  # Problem-solving aspects
    value_conflicts: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Conflicting value systems
    institutional_support: List[uuid.UUID] = field(
        default_factory=lambda: []
    ...
    influenced_matrix_cells: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Matrix cells influenced
    legitimacy_source: Optional[str] = None  # What legitimizes this value system
    stakeholder_alignment: Dict[str, float] = field(
        default_factory=lambda: {}
...
class SocialBelief(Node):
    ...
    supporting_values: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Values that support this belief
    conflicting_beliefs: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Contradictory beliefs
    institutional_embedment: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Institutions embedding this belief
    ...
    # Matrix integration
    institutional_influence: Dict[uuid.UUID, float] = field(
        default_factory=lambda: {}
    )  # Institution -> influence level
    attitude_mediation_effects: Dict[uuid.UUID, float] = field(
        default_factory=lambda: {}
    )  # Attitude -> mediation effect
...
class CulturalAttitude(Node):
    ...
    related_beliefs: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Beliefs this attitude relates to
    influenced_institutions: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Institutions influenced by attitude
    attitude_objects: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Objects of the attitude

    # Attitude characteristics
    formation_context: Optional[str] = None  # Context where attitude formed
    stability_factors: List[str] = field(
        default_factory=lambda: []
    )  # Factors supporting attitude stability
    change_triggers: List[str] = field(
        default_factory=lambda: []
    )  # Potential change triggers
    social_desirability: Optional[float] = None  # Social acceptability (0-1)

    # Matrix integration
    institutional_mediation_effects: Dict[uuid.UUID, float] = field(
        default_factory=lambda: {}
    )  # Institution -> mediation
    belief_attitude_coherence: Optional[float] = None  # Coherence with beliefs (0-1)
    behavioral_predictability: Optional[float] = (
        None  # Predictability of behavior (0-1)
    )
    ...
    def analyze_mediation_capacity(self) -> Dict[str, Any]:
        ...
        if (
            self.attitude_strength is not None
            and self.behavioral_predictability is not None
        ):
            mediation_analysis["mediation_strength"] = (
                self.attitude_strength + self.behavioral_predictability
            ) / 2
        ...
        # Assess stability
        stability_score = len(self.stability_factors) / max(
            1, len(self.change_triggers)
        )

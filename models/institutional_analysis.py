...
class InstitutionalStructure(Node):
    ...
    structure_type: str = (
        "formal"  # Made string with default instead of enum to fix ordering
    )
    governance_mechanism: str = (
        "hierarchical"  # Made string with default instead of enum
    )
    decision_making_process: str = (
        "autocratic"  # Made string with default instead of enum
    )
    ...
    power_distribution: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # How power is distributed
    accountability_mechanisms: List[str] = field(
        default_factory=lambda: []
    )  # How institution is held accountable
    change_mechanisms: List[str] = field(
        default_factory=lambda: []
    )  # How institution can change
    formal_rules: List[str] = field(default_factory=lambda: [])  # Written rules
    informal_norms: List[str] = field(
        default_factory=lambda: []
...
class PathDependencyAnalysis(Node):
    ...
    analyzed_institution_id: Optional[uuid.UUID] = (
        None  # Made optional for dataclass ordering
    )
    dependency_strength: PathDependencyType = PathDependencyType.MODERATE
    critical_junctures: List[str] = field(
        default_factory=lambda: []
    )  # Key historical moments
    lock_in_mechanisms: List[str] = field(
        default_factory=lambda: []
    ...
    def __post_init__(self) -> None:
        ...
        if self.analyzed_institution_id is None:
            raise ValueError(
                "analyzed_institution_id is required for PathDependencyAnalysis"
            )

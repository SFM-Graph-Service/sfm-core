...
class InstrumentalistInquiryFramework(Node):
    ...
    embedded_values: List[str] = field(
        default_factory=lambda: []
    )  # Values embedded in the inquiry
    ...
    evolutionary_perspective: Optional[float] = (
        None  # Evolutionary approach level (0-1)
    )
    holistic_approach: Optional[float] = None  # Holistic thinking level (0-1)

    # Inquiry process
    inquiry_stages: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Stages of inquiry
    knowledge_integration: Dict[str, List[str]] = field(
        default_factory=lambda: {}
    )  # How knowledge is integrated
    stakeholder_involvement: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Stakeholder participation

    # Methodological principles
    fallibilistic_approach: Optional[bool] = None  # Recognition of fallibility
    democratic_participation: Optional[float] = (
        None  # Level of democratic participation (0-1)
    )
...
class NormativeSystemsAnalysis(Node):
    ...
    normative_criteria: List[str] = field(
        default_factory=lambda: []
    )  # Normative evaluation criteria
    value_hierarchy: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # Value priorities
    ...
    # Analysis components
    system_evaluation: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # System component evaluations
    alternative_assessments: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Alternative system assessments
    policy_recommendations: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    ...
    # Evaluation process
    stakeholder_values: Dict[str, Dict[str, float]] = field(
        default_factory=lambda: {}
    )  # Stakeholder value sets
    value_conflicts: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Identified value conflicts
    consensus_mechanisms: List[str] = field(
        default_factory=lambda: []
    ...
    def conduct_normative_evaluation(self) -> Dict[str, Any]:
        ...
        evaluation_results["overall_system_score"] = (
        ...
        evaluation_results["stakeholder_consensus_level"] = (
            self._calculate_stakeholder_consensus()
        )
        evaluation_results["normative_recommendations"] = (
            self._generate_normative_recommendations(evaluation_results)
        )
    ...
    def _calculate_stakeholder_consensus(self) -> float:
        ...
        for value_type in self.value_hierarchy:
            ...
            if len(stakeholder_scores) > 1:
                ...
                variance = sum((s - mean_score) ** 2 for s in stakeholder_scores) / len(
                    stakeholder_scores
                )
    ...
    def _generate_normative_recommendations(
...
class PolicyRelevanceIntegration(Node):
    ...
    lobbying_strategies: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Lobbying approaches
    budgetary_processes: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Budget integration
    administrative_implementation: Dict[str, List[str]] = field(
        default_factory=lambda: {}
    )  # Implementation pathways
    legislative_pathways: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Legislative routes

    # Policy tools
    policy_instruments: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Available policy tools
    institutional_leverage_points: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Leverage points
    coalition_building_opportunities: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    ...
    def assess_policy_integration_capacity(self) -> Dict[str, Any]:
        ...
        integration_assessment["overall_integration_capacity"] = (
            self._calculate_integration_capacity()
        )
        integration_assessment["political_viability"] = (
            self._assess_political_viability()
        )
        integration_assessment["implementation_readiness"] = (
            self._assess_implementation_readiness()
        )
        integration_assessment["stakeholder_alignment"] = (
            self._assess_stakeholder_alignment()
        )
        integration_assessment["strategic_recommendations"] = (
    ...
    def _generate_strategic_recommendations(self) -> List[str]:
        ...
        if (
            self.implementation_capacity is not None
            and self.implementation_capacity < 0.4
        ):
...
class DatabaseIntegrationCapability(Node):
    ...
    data_quality_standards: List[str] = field(
        default_factory=lambda: []
    )  # Quality standards

    # Data management
    data_collection_protocols: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Collection protocols
    data_validation_rules: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Validation rules
    data_storage_systems: Dict[str, str] = field(
        default_factory=lambda: {}
    )  # Storage systems
    data_access_controls: List[str] = field(
        default_factory=lambda: []
    )  # Access controls

    # Statistical analysis integration
    statistical_packages: List[str] = field(
        default_factory=lambda: []
    )  # Statistical software
    analysis_workflows: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Analysis workflows
    visualization_capabilities: Dict[str, List[str]] = field(
        default_factory=lambda: {}
    )  # Visualization tools
    reporting_systems: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # Reporting capabilities

    # Matrix integration
    matrix_data_mapping: Dict[str, str] = field(
        default_factory=lambda: {}
    )  # Matrix -> database mapping
    indicator_calculation_rules: Dict[str, Dict[str, Any]] = field(
        default_factory=lambda: {}
    )  # Calculation rules
    automated_updates: List[str] = field(
        default_factory=lambda: []
    ...
    def evaluate_database_capability(self) -> Dict[str, Any]:
        ...
        quality_indicators = len(self.data_quality_standards) + len(
        ...
        if quality_indicators >= 5:
            ...
            capability_evaluation["improvement_priorities"].append(
                "Strengthen data management"
            )

        # Assess analytical power
        analysis_capabilities = len(self.statistical_packages) + len(
        ...
        integration_indicators = len(self.matrix_data_mapping) + len(
            self.indicator_calculation_rules
        )

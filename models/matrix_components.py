...
class MatrixCell(Node):  # pylint: disable=too-many-instance-attributes
    ...
    institution_id: Optional[uuid.UUID] = (
        None  # Made optional with default to fix dataclass ordering  # pylint: disable=line-too-long
    )
    criteria_id: Optional[uuid.UUID] = (
        None  # Made optional with default to fix dataclass ordering  # pylint: disable=line-too-long
    )
    ...
    )  # Hayden's standardized -3 to +3 scale
    ...
    deliveries_provided: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # What this cell delivers to others
    deliveries_received: List[Dict[str, Any]] = field(
        default_factory=lambda: []
    )  # What this cell receives from others
    ...
    cultural_values_influence: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # Value type -> influence strength
    social_beliefs_alignment: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # Belief type -> alignment level
    attitude_mediation: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # Attitude type -> mediation effect
    cultural_legitimacy: Optional[float] = (
        None  # Cultural legitimacy of the relationship (0-1)
    )

    # Ceremonial vs Instrumental Analysis
    ceremonial_component: Optional[float] = (
        None  # Ceremonial aspects of the relationship (0-1)
    )
    instrumental_component: Optional[float] = (
        None  # Instrumental aspects of the relationship (0-1)
    )
    ceremonial_barriers: List[str] = field(
        default_factory=lambda: []
    )  # Ceremonial obstacles
    instrumental_enablers: List[str] = field(
        default_factory=lambda: []
    )  # Instrumental facilitators

    # Technology Integration
    technology_dependencies: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Required technologies
    technology_compatibility: Optional[float] = (
        None  # How well technology supports relationship (0-1)
    )
    technological_change_sensitivity: Optional[float] = (
        None  # Sensitivity to tech changes (0-1)
    )
    ...
    resource_flows: Dict[str, float] = field(
        default_factory=lambda: {}
    )  # Resource type -> flow amount

    # Network Process Integration
    network_position: Optional[str] = (
        None  # Position in network ("central", "peripheral", "bridge")
    )
    ...
    social_indicators: Dict[str, float] = field(
        default_factory=lambda: {}
    ...
    sequence_dependencies: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Dependent cells in sequence
    temporal_coordination_requirements: List[str] = field(
        default_factory=lambda: []
    )  # Timing requirements
    ...
    def assess_cell_quality(self) -> Dict[str, Any]:
        ...
        # Assess data quality
        if len(self.data_sources) >= 3 and self.evidence_quality in [
            ...
            and self.evidence_quality == EvidenceQuality.MEDIUM
        ):
        ...
        # Assess completeness
        required_fields: list[Optional[Any]] = [
            self.institution_id,
            self.criteria_id,
            self.justification,
        ]
    ...
    def analyze_delivery_relationships(self) -> Dict[str, Any]:
        ...
        if self.delivery_quality and self.delivery_reliability:
            delivery_analysis["delivery_effectiveness"] = (
                self.delivery_quality + self.delivery_reliability
            ) / 2
        ...
        if self.system_criticality and self.network_influence:
            delivery_analysis["delivery_criticality"] = (
                self.system_criticality + self.network_influence
            ) / 2
    ...
    def assess_cultural_integration(self) -> Dict[str, Any]:
        ...
        if total_cultural_elements > 0:
            values_avg = (
                sum(self.cultural_values_influence.values())
                / len(self.cultural_values_influence)
                if self.cultural_values_influence
                else 0
            )
            beliefs_avg = (
                sum(self.social_beliefs_alignment.values())
                / len(self.social_beliefs_alignment)
                if self.social_beliefs_alignment
                else 0
            )
            attitudes_avg = (
                sum(self.attitude_mediation.values()) / len(self.attitude_mediation)
                if self.attitude_mediation
                else 0
            )
            ...
            if len([s for s in cultural_scores if s > 0]) > 1:
                variance = sum(
                    (s - sum(cultural_scores) / 3) ** 2 for s in cultural_scores
                ) / len(cultural_scores)
    ...
    def analyze_ceremonial_instrumental_balance(self) -> Dict[str, Any]:
        ...
        if ceremonial > instrumental * 1.5:
            ...
            balance_analysis["adjustment_recommendations"].append(
                "Increase instrumental problem-solving capacity"
            )
            ...
            balance_analysis["adjustment_recommendations"].append(
                "Consider ceremonial stabilization needs"
            )
        else:
        ...
        if len(self.ceremonial_barriers) > len(self.instrumental_enablers):
            ...
            balance_analysis["adjustment_recommendations"].append(
                "Leverage instrumental enablers"
            )
    ...
    def assess_ecological_integration(self) -> Dict[str, Any]:
        ...
        if (
            self.ecological_impact is not None
            and self.ecological_sustainability is not None
        ):
...
class SFMCriteria(Node):  # pylint: disable=too-many-instance-attributes
    ...
    )  # Hayden's primary/secondary distinction
    ...
    life_process_relevance: Optional[float] = (
        None  # Relevance to life process enhancement (0-1)
    )
    instrumental_capacity: Optional[float] = (
        None  # Problem-solving capacity measure (0-1)
    )
    ceremonial_bias_risk: Optional[float] = None  # Risk of ceremonial distortion (0-1)
    normative_justification: Optional[str] = (
        None  # Why this criterion matters normatively
    )
    ...
    def assess_criterion_quality(self) -> Dict[str, Any]:
        ...
        if (
            self.priority == CriteriaPriority.PRIMARY
            and (self.life_process_relevance or 0) < 0.5
        ):
            assessment["priority_appropriateness"] = "questionable"

        # Check measurement feasibility
        if (
            self.measurement_approach == MeasurementApproach.QUANTITATIVE
            and not self.data_requirements
        ):
        ...
        if self.normative_justification:
            ...
            not self.normative_justification
            and self.priority == CriteriaPriority.PRIMARY
        ):
...
class SFMMatrix(Node):  # pylint: disable=too-many-instance-attributes
    ...
    matrix_cells: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # All cells in matrix
    ...
    cross_impact_analyses: List[uuid.UUID] = field(
        default_factory=lambda: []
    )  # Links to CrossImpactAnalysis
    problem_solving_sequences: List[uuid.UUID] = field(
        default_factory=lambda: []
